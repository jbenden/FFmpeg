...
static int add_video_device(AVFormatContext *s, AVCaptureDeviceInput *video_device)
{
    AVFContext *ctx = (AVFContext*)s->priv_data;
    //NSError *error  = nil;

    if ([ctx->capture_session canAddInput:video_device]) {
        [ctx->capture_session addInput:video_device];
    } else {
    ...
    if (ctx->audio_device_index >= 0) {
        ...
        for (AVCaptureDevice *device in devices) {
            if (!strncmp(ctx->audio_filename, [[device localizedName] UTF8String], strlen(ctx->audio_filename))) {
                audio_device = device;
                av_log(ctx, AV_LOG_INFO, "Using audio device %s\n", [[device localizedName] UTF8String]);
                break;
            }
        }
